try {
    ...
    Invoke-RestMethod -Uri "$serverUrl/get_mining_registry" `
        -Method 'Get' `
        -Headers @{'token' = $Env:SERVER_TOKEN } `
        -OutFile "message_mining_registry_downloaded.json"
    Write-Host "Message mining registry downloaded successfully."
} catch {
    Write-Host "Failed to download message mining registry."
    Write-Host $_
} finally {
